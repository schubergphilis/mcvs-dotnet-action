...
#      FETCH_ARGUMENT:  The argument passed to FETCH_FUNCTION (GitHub repo or nuget package)
...
readonly PACKAGES_TO_BE_UPDATED=(
  ...
  "REPORT_GENERATOR_VERSION REPORT_GENERATOR_VERSION reportgenerator latest_stable_package_version_on_nuget dotnet-reportgenerator-globaltool"
  "STRYKER_VERSION STRYKER_VERSION stryker latest_stable_package_version_on_nuget dotnet-stryker"
...
# The .NET tools, such as the dotnet-reportgenerator-globaltool, are installed
# from nuget.org rather than from the GitHub releases of the repository that
# hosts the source code. For those, nuget is the source of truth.
latest_stable_package_version_on_nuget() {
  local package="$1"

  curl -sSfL "https://api.nuget.org/v3-flatcontainer/${package,,}/index.json" | \
    jq -r '
